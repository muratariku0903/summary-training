...
export const exerciseEvaluationStatus = pgEnum('exercise_evaluation_status', [
  'queued',
  'processing',
  'succeeded',
  'failed',
])
export const exerciseOutputDataType = pgEnum('exercise_output_data_type', [
  'text',
  'audio',
  'text/audio',
])
export const exerciseOutputDifficulty = pgEnum('exercise_output_difficulty', [
  'easy',
  'medium',
  'hard',
])
export const exerciseOutputExerciseType = pgEnum('exercise_output_exercise_type', [
  'summary',
  'rewrite',
])
...
export const seedGeneratorThemeCreatedType = pgEnum('seed_generator_theme_created_type', [
  'system',
  'admin',
])
export const seedProfileType = pgEnum('seed_profile_type', [
  'ai_theme',
  'youtube_channels',
  'web',
  'storage',
...
export const seedGeneratorProfiles = pgTable(
  ...
  (table) => [
    index('idx_sgp_active').using(
      'btree',
      table.isActive.asc().nullsLast().op('bool_ops'),
    ),
    index('idx_sgp_type').using(
      'btree',
      table.profileType.asc().nullsLast().op('enum_ops'),
    ),
  ],
)

export const seedGeneratorThemes = pgTable(
  ...
  (table) => [
    index('idx_sgt_name_trgm').using(
      'gin',
      table.title.asc().nullsLast().op('gin_trgm_ops'),
    ),
    uniqueIndex('uidx_sgt_canonical_key').using(
      'btree',
      table.canonicalKey.asc().nullsLast().op('text_ops'),
    ),
  ],
)
...
export const exerciseGeneratorSeeds = pgTable(
  'exercise_generator_seeds',
  {
    ...
    llmId: uuid('llm_id'),
    rawTextTrgmGenerated: text('raw_text_trgm_generated').generatedAlwaysAs(
      sql`"left"(_trgm_normalize(raw_text), 3000)`,
    ),
  },
  (table) => [
    index('idx_egs_generator_profile').using(
      'btree',
      table.generatorProfileId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_egs_raw_text_trgm').using(
      'gin',
      table.rawTextTrgmGenerated.asc().nullsLast().op('gin_trgm_ops'),
    ),
    index('idx_egs_title_trgm').using(
      'gin',
      table.title.asc().nullsLast().op('gin_trgm_ops'),
    ),
    foreignKey({
...
export const idpLinks = pgTable(
  ...
  (table) => [
    index('idx_idp_links_auth_user_id').using(
      'btree',
      table.authUserId.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_idp_links_provider_external').using(
      'btree',
      table.provider.asc().nullsLast().op('text_ops'),
      table.externalUserId.asc().nullsLast().op('text_ops'),
    ),
    foreignKey({
    ...
    unique('idp_links_provider_external_user_id_key').on(
      table.provider,
...
export const exercises = pgTable(
  ...
  (table) => [
    index('idx_exercises_created_by').using(
      'btree',
      table.createdBy.asc().nullsLast().op('uuid_ops'),
    ),
    index('idx_exercises_status_created_at').using(
      'btree',
    ...
    pgPolicy('read_own_non_ready', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
      using: sql`(created_by = auth.uid())`,
    }),
    pgPolicy('read_ready_exercises_auth', {
      as: 'permissive',
      for: 'select',
      to: ['authenticated'],
    }),
    check(
      'exercises_title_check',
      sql`(char_length(title) >= 1) AND (char_length(title) <= 120)`,
    ),
  ],
)
...
export const seedGeneratorCategories = pgTable(
...
export const exerciseEvaluationDetails = pgTable(
  ...
  (table) => [
    index('exercise_evaluation_details_evaluation_id_idx').using(
      'btree',
      table.evaluationId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const exerciseSubmissions = pgTable(
  ...
  (table) => [
    index('exercise_submissions_exercise_id_idx').using(
      'btree',
      table.exerciseId.asc().nullsLast().op('uuid_ops'),
    ),
    index('exercise_submissions_user_id_idx').using(
      'btree',
...
export const exerciseGeneratorSources = pgTable(
  ...
  (table) => [
    uniqueIndex('exercise_generator_sources_theme_key')
      .using('btree', table.themeId.asc().nullsLast().op('uuid_ops'))
      .where(sql`(theme_id IS NOT NULL)`),
    foreignKey({
      columns: [table.themeId],
      foreignColumns: [seedGeneratorThemes.id],
      name: 'exercise_generator_sources_theme_id_fkey',
    }),
    unique('exercise_generator_sources_theme_aggregate_unique').on(
      table.themeId,
      table.aggregateType,
    ),
  ],
...
export const exerciseGeneratorProfileSourcePatterns = pgTable(
  'exercise_generator_profile_source_patterns',
  {
    id: uuid().defaultRandom().primaryKey().notNull(),
    profileId: uuid('profile_id').notNull(),
    sourceIds: uuid('source_ids').array().notNull(),
    sourceSetKey: text('source_set_key').generatedAlwaysAs(
      sql`uuid_array_sorted_key(source_ids)`,
    ),
  ...
  (table) => [
    index('idx_profile_patterns_last_used').using(
      'btree',
      table.profileId.asc().nullsLast().op('timestamptz_ops'),
      table.lastUsedAt.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const exerciseGeneratorOutputConfigs = pgTable(
  ...
  (table) => [
    ...
    unique('exercise_generator_output_configs_uni4').on(
      table.llmId,
      table.dataType,
      table.exerciseType,
      table.difficulty,
...
export const exerciseGeneratorProfiles = pgTable(
  'exercise_generator_profiles',
  {
    ...
    allowRepeatWhenExhausted: boolean('allow_repeat_when_exhausted')
      .default(false)
      .notNull(),
  },
  (table) => [
    index('idx_ex_gen_profiles_output_config').using(
      'btree',
      table.outputConfigId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
...
export const exerciseEvaluations = pgTable(
  ...
  (table) => [
    index('exercise_evaluations_status_idx').using(
      'btree',
      table.status.asc().nullsLast().op('enum_ops'),
    ),
    index('exercise_evaluations_submission_id_idx').using(
      'btree',
...
export const seedGeneratorThemeCategories = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.themeId, table.categoryId],
      name: 'seed_generator_theme_categories_pkey',
    }),
  ],
)

export const exerciseGeneratorSourceSeeds = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.sourceId, table.seedId],
      name: 'exercise_generator_source_seeds_pkey',
    }),
  ],
)

export const exerciseGeneratorProfileSources = pgTable(
  ...
  (table) => [
    ...
    primaryKey({
      columns: [table.profileId, table.sourceId],
      name: 'exercise_generator_profile_sources_pkey',
    }),
  ],
)

export const exerciseGeneratorOutputConfigsSchemas = pgTable(
  ...
  (table) => [
    index('idx_ex_out_cfg_schema_llm').using(
      'btree',
      table.llmId.asc().nullsLast().op('uuid_ops'),
    ),
    foreignKey({
      columns: [table.llmId],
      foreignColumns: [llms.id],
      ...
      .onDelete('restrict'),
    primaryKey({
      columns: [table.llmId, table.dataType, table.exerciseType, table.difficulty],
      name: 'exercise_generator_output_configs_schemas_pkey',
    }),
    check(
      'exercise_generator_output_configs_schemas_schema_is_object',
      sql`jsonb_typeof(schema) = 'object'::text`,
    ),
  ],
)

export const exerciseEvaluationRubrics = pgTable(
