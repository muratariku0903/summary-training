...
drop function if exists public.pick_random_unused_source_pattern(uuid, int, int, int, bool);
...
returns table(
  pattern_id      uuid,                  -- 挿入されたパターンのID（重複許可時はNULL）
...
begin
  ...
  if p_allow_duplicates then
    ...
    if v_sid is not null and cardinality(v_sid) = v_k then
      ...
      pattern_id      := null;  -- 重複許可時はパターンIDはNULL
      source_ids      := v_sid;
      return next;
    end if;
  ...
  while i < coalesce(p_max_attempts, 10) loop
    ...
    if v_id is not null then
      pattern_id      := v_id;   -- 挿入されたパターンのIDを返却
      source_ids      := v_sid;
      return next;
      return;
    end if;
